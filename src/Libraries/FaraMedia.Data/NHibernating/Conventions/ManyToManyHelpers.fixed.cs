	...
	public static class ManyToManyHelpers {
		public static PropertyInfo GetInverseProperty(this PropertyInfo propertyInfo) {
			...
			var expectedInversePropertyType = type.GetGenericTypeDefinition().
			                                       MakeGenericType(propertyInfo.DeclaringType);

			var argument = type.GetGenericArguments()[0];

			return argument.GetProperties().
			                FirstOrDefault(pi => pi.PropertyType == expectedInversePropertyType && pi != propertyInfo);
		}
		...
		public static PropertyInfo GetInverseProperty(this MemberInfo memberInfo) {
			...
			var expectedInversePropertyType = type.GetGenericTypeDefinition().
			                                       MakeGenericType(memberInfo.DeclaringType);

			var argument = type.GetGenericArguments()[0];

			return argument.GetProperties().
			                FirstOrDefault(pi => pi.PropertyType == expectedInversePropertyType);
		}
	}
}